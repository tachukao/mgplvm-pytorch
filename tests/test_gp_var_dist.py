...
def test_GP_lat_prior():
    ...
    for nGP, GP in enumerate([mgp.rdist.GP_diag, mgp.rdist.GP_circ]):
        ...
        _scale = torch.ones(n_samples, d, m) * 0.5 * (1 + torch.randn(m) / 100)  #n_diag x T
        ...
        kl = mod.lat_dist.kl() #computed KL
        Kfull = mod.lat_dist.full_cov().to(device) #computed full covariance (n_samples x d x m x m)
        mu = mod.lat_dist.lat_mu.transpose(-1, -2).to(device) #(n_samples x d x m)

        lhalf = lat_dist.ell / np.sqrt(2)
        sighalf_sq = 1 * 2**(1/4) * np.pi**(-1/4) * lat_dist.ell**(-1/2)

        print(lhalf.shape, sighalf_sq.shape, dts_sq.shape)

        Khalf_prior = sighalf_sq[..., None] * torch.exp( - torch.tensor(dts_sq[:, None, ...]).to(device) / (2*lhalf[..., None]**2) ) 
        Kprior = Khalf_prior @ Khalf_prior #(n_samples x 1 x m x m)

        print(Kfull.shape, Kprior.shape, mu.shape)

        p = torch.distributions.multivariate_normal.MultivariateNormal(torch.zeros(1).to(device), covariance_matrix=(Kprior+jitter))
        q = torch.distributions.multivariate_normal.MultivariateNormal(mu, covariance_matrix=(Kfull+jitter))
