...
def test_GP_prior():
    ...
    #### naive computation ####
    LLs1 = [mod.lprior.svgp.elbo(1, 1, x[i, :, :, None].permute(1,0,2), ts.reshape(1,1,-1)) for i in range(x.shape[0])]
    lik1 = torch.stack([LL[0] for LL in LLs1], dim = 0).sum() #sum likelihood across samples of x
    kl1 = torch.stack([LL[1] for LL in LLs1], dim = 0).sum() #sum KL across samples of x

    #### try to batch things ####
    lik2, kl2 = mod.lprior.svgp.elbo(1, x.shape[0], x.permute(2, 1, 0), ts.reshape(1, 1,-1))
