...
timepoints = np.arange(
    3000,
    6000)  #subsample 75 seconds of data so things will run somewhat quicker
fit_data = {
    'Y': data['Y'][..., timepoints],
    'locs': data['locs'][timepoints, :],
    'targets': data['targets'][timepoints, :],
    'binsize': binsize
}
Y = fit_data[
    'Y']  # these are the actual recordings and is the input to our model

Y = Y[:, np.mean(Y, axis=(0, 2)) / 0.025 >
      6, :]  #subsample highly active neurons so things will run a bit quicker
...
lik = mgp.likelihoods.NegativeBinomial(
    n, Y=Y
)  # we use a negative binomial noise model in this example (recommended for ephys data)
...
var_dist = mgp.rdist.GP_circ(
    manif, T, ntrials, fit_ts, _scale=1, ell=ell0
