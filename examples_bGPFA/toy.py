...
p_nb = np.exp(F + c_nb) / (1 + np.exp(F + c_nb)
                          )  #probability of failure (c.f. Jensen & Kao et al.)
r_nb = np.random.uniform(
    1, 10,
    n)  # number of failures (overdispersion paramer; c.f. Jensen & Kao et al.)
...
def cb_ard(mod, i, loss):
    ...
    if i % 100 == 0:
        ...
        print('\niter:', i, 'time:',
              str(round(time.time() - t0)) + 's', 'scales:',
...
Y = YNB[
    None,
    ...]  # we add a trial dimension which isn't actually used here but could be if we had multiple trials
data = torch.tensor(Y).to(device)  # put the data on our GPU/CPU
fit_ts = torch.tensor(ts)[None,
                          None, :].to(device)  # put our time points on GPU/CPU
...
lik = mgp.likelihoods.NegativeBinomial(
    n, Y=Y)  # we use a negative binomial noise model in this example
manif = mgp.manifolds.Euclid(
    T, d_fit
)  # our latent variables live in a Euclidean space (c.f. Jensen et al. 2020)
...
var_dist = mgp.rdist.GP_circ(
    manif, T, 1, fit_ts, _scale=1,
    ell=20 * 0.8)  # circulant variational posterior (c.f. Jensen & Kao et al.)
mod = mgp.models.Lvgplvm(n,
                         ...
                         learn_scale=False,
                         ard=True,
                         rel_scale=0.1).to(device)  #create ARD model
...
train_ps = mgp.crossval.training_params(
    ...
    lrate=5e-2,
    callback=cb_ard,
    print_every=100)  # helper function to specify training parameters
...
nus = np.sqrt(np.mean(detach(mod.lat_dist.nu)**2,
                      axis=(0, -1)))  #magnitude of the variational mean
plt.figure()
plt.scatter(dim_scales, nus, c='k', marker='x',
            s=30)  #top right corner are informative, lower left discarded
...
lats = lats[..., np.argsort(
    -dim_scales
)[:
  2]]  #only consider the two most informative dimensions (c.f. Jensen & Kao et al.)
...
T = np.linalg.inv(
    lats.T @ lats) @ lats.T @ true_lats  #regress onto ground truth latents (xs)
...
id_line = np.log(np.array([np.amin(r_nb),
                           np.amax(r_nb)]))  #plot identity line for reference
