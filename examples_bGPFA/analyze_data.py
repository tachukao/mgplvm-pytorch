...
nus = np.sqrt(np.mean(mod.lat_dist.nu.detach().cpu().numpy()**2,
...
X = mod.lat_dist.lat_mu.detach().cpu().numpy()[
    0, ...]  #extract inferred latents ('mu' has shape (ntrials x T x d_fit))
X = X[..., np.argsort(
    -dim_scales
)[:2]]  #only consider the two most informative dimensions (c.f. Jensen & Kao)
tplot = np.arange(
    100, 200
)  #let's only plot a shorter period (here 2.s) so it doesn't get too cluttered

#fit FA for comparison
fa = FactorAnalysis(2)
Y = fit_data['Y']
Xfa = fa.fit_transform(np.sqrt(Y[
    0,
    ...].T))  #sqrt the counts for variance normalization (c.f. Yu et al. 2009)
...
Ypreds = [
]  #decode from the inferred firing rates (this is a non-linear decoder from latents)
query = mod.lat_dist.lat_mu.detach().transpose(-1, -2).to(
    device)  #(ntrial, d_fit, T)
...
for idelay, delay in enumerate(delays):
    ...
    regs = [LinearRegression().fit(Ypred, vels[:, i]) for i in range(2)
           ]  #fit x and y vel
    scores = [regs[i].score(Ypred, vels[:, i]) for i in range(2)
             ]  #score x and y vel
